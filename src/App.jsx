        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
          ...
          <button onClick={onImport} className="btn-icon" title="Import JSON" style={{
            ...
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
              <path d="M6 1v6M3 5l3 3 3-3M2 10h8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
            </svg>
            {!mobile&&"Import"}
          </button>
          {/* Export JSON */}
          <button onClick={onExport} className="btn-icon" title="Export JSON" style={{
            background:"rgba(255,255,255,0.05)",
            border:"1px solid rgba(30,52,90,0.7)",
            color:"rgba(255,255,255,0.6)",
            padding:mobile?"7px":"7px 11px",
            ...
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
              <path d="M6 8V2M3 4l3-3 3 3M2 10h8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
            </svg>
            {!mobile&&"Export"}
          </button>
...
function Chip({entry,onEdit,onDelete,onSegClick,isSegSection,wide,idx=0}){
  ...
  return(
    ...
    >
      ...
      {/* Row 3: author tag (shown only when present) */}
      {entry.author&&(
        <span style={{
...
function VisualMap({state,segId,seg}){
  ...
  const sections = FLOW.map(({sid,color,label})=>({
    sid, color, label,
    items:(state[sid]||[]).filter(e=>(e.segs||[]).includes(segId)),
  ...
  if(total===0){
    return(
      <div style={{padding:"40px 16px",textAlign:"center",opacity:.45}}>
        <div style={{fontSize:30}}>🗺</div>
        <div style={{fontSize:11,color:"rgba(255,255,255,0.5)",marginTop:10,lineHeight:1.6}}>
          No entries linked yet.<br/>Link entries to this segment to populate the visual map.
        </div>
      </div>
    );
  }

  // Layout: sections arranged radially around the segment node, entries as
  // satellite nodes around each section node.
  const sectionCount = sections.length;
  const sectionRadius = 200;
  const entryRadius   = 78;
    ...
    <radialGradient id="vm-glow" cx="50%" cy="50%" r="50%">
      ...
      <stop offset="100%" stopColor="#ffd100" stopOpacity="0"/>
         ...
         <g transform={`translate(${pan.x},${pan.y}) scale(${zoom})`}>
          ...
          {/* connection lines: segment → section, section → entry */}
          {sectionPositions.map(sec=>{
            const secActive = hoveredSection===sec.sid || hoveredEntry?.sid===sec.sid;
            return (
              <line key={"seg-"+sec.sid}
                x1={cx} y1={cy} x2={sec.x} y2={sec.y}
                stroke={sec.color}
                strokeWidth={secActive?2.4:1.4}
                strokeOpacity={secActive?0.85:0.4}
          ...
          {sectionPositions.map(sec=>
            sec.items.map((e,i)=>{
              ...
              const spread = Math.min(0.9, 0.32 + n*0.07);
              ...
              return(
                <g key={e.id}>
                  <line x1={sec.x} y1={sec.y} x2={ex} y2={ey}
                    stroke={sec.color}
                    strokeWidth={isHover?2:1}
                    strokeOpacity={isHover?0.9:0.32}
                    style={{transition:"stroke-width .15s, stroke-opacity .15s"}}/>
                </g>
              );
            })
          ...
          {/* segment node (centre) */}
          <g>
            <circle cx={cx} cy={cy} r={34} fill="#06090f" stroke={Y} strokeWidth="2.5"/>
            <circle cx={cx} cy={cy} r={34} fill={Y} fillOpacity="0.12"/>
            <text x={cx} y={cy-2} textAnchor="middle" fontSize="9" fontFamily="monospace" fontWeight="800" fill={Y}>👤</text>
            <text x={cx} y={cy+13} textAnchor="middle" fontSize="6.5" fontFamily="monospace" fill="rgba(255,255,255,0.7)">SEGMENT</text>
          </g>
          <text x={cx} y={cy+52} textAnchor="middle" fontSize="9.5" fontWeight="700" fill="#fff">
            {seg.text.length>34 ? seg.text.slice(0,34)+"…" : seg.text}
          </text>

          {/* section nodes */}
          {sectionPositions.map(sec=>{
            const secHover = hoveredSection===sec.sid || hoveredEntry?.sid===sec.sid;
            return(
              <g key={sec.sid}
                onMouseEnter={()=>setHoveredSection(sec.sid)}
                onMouseLeave={()=>setHoveredSection(null)}
                style={{cursor:"default"}}>
                <circle cx={sec.x} cy={sec.y} r={secHover?24:21}
                  fill={sec.color} fillOpacity={secHover?0.28:0.16}
                  stroke={sec.color} strokeWidth={secHover?2.2:1.6}
                  style={{transition:"r .15s, fill-opacity .15s, stroke-width .15s"}}/>
                <text x={sec.x} y={sec.y+3} textAnchor="middle" fontSize="9" fontFamily="monospace" fontWeight="800" fill="#fff">
                  {sec.items.length}
                </text>
                <text x={sec.x} y={sec.y + (secHover?40:36)} textAnchor="middle" fontSize="8" fontWeight="700" fill={sec.color}
                  style={{transition:"y .15s"}}>
                  {sec.label}
                </text>
              </g>
            );
          ...
          {sectionPositions.map(sec=>
            sec.items.map((e,i)=>{
              ...
              return(
                <g key={e.id}
                  onMouseEnter={()=>setHoveredEntry({...e,sid:sec.sid})}
                  onMouseLeave={()=>setHoveredEntry(null)}
                  style={{cursor:"pointer"}}>
                  <circle cx={ex} cy={ey} r={isHover?8:6}
                    fill={eColor} stroke="#06090f" strokeWidth="1.5"
                    style={{transition:"r .15s",filter:isHover?`drop-shadow(0 0 6px ${eColor})`:"none"}}/>
                </g>
              );
            })
          )}
          ...
          {/* hover tooltip */}
          {hoveredEntry&&(()=>{
            const sec = sectionPositions.find(s=>s.sid===hoveredEntry.sid);
            const n = sec.items.length;
            const i = sec.items.findIndex(e=>e.id===hoveredEntry.id);
            const spread = Math.min(0.9, 0.32 + n*0.07);
            const localAngle = sec.angle + (i-(n-1)/2) * (spread/Math.max(n-1,1) || 0);
            const ex = sec.x + Math.cos(localAngle)*entryRadius;
            const ey = sec.y + Math.sin(localAngle)*entryRadius*0.85;
            const text = hoveredEntry.text.length>60 ? hoveredEntry.text.slice(0,60)+"…" : hoveredEntry.text;
            const boxW = Math.min(260, Math.max(120, text.length*5.2));
            const boxX = Math.min(Math.max(ex-boxW/2, 6), W-boxW-6);
            const boxY = ey>H-70 ? ey-46 : ey+14;
            return(
...
function StoryDrawer({state,activeSegId,onSelectSeg,onClose}){
  ...
  const [view,setView]=useState("overview"); // "overview" | "map"
  ...
  return(
    <div style={{background:"rgba(4,7,16,0.95)",backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",borderLeft:"1px solid rgba(30,52,90,0.65)",display:"flex",flexDirection:"column",height:"100%",overflow:"hidden",animation:"slideIn 0.25s ease"}}>
      ...
      {/* View switcher — only shown once a segment is selected */}
      {seg&&(
        <div style={{display:"flex",gap:6,padding:"10px 16px 0",flexShrink:0}}>
          {[
            {id:"overview",label:"Overview",icon:"📊"},
            {id:"map",     label:"Visual Map",icon:"🕸"},
          ].map(t=>(
            <button key={t.id} onClick={()=>setView(t.id)}
              style={{
                ...
                background:view===t.id?"rgba(255,209,0,0.1)":"rgba(255,255,255,0.03)",
                border:`1px solid ${view===t.id?"rgba(255,209,0,0.45)":"rgba(30,52,90,0.6)"}`,
                color:view===t.id?Y:"rgba(255,255,255,0.55)",
                transition:"all .15s",
              }}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"12px 16px",display:"flex",flexDirection:"column",gap:10}}>
        {!activeSegId?(
          ...
          <>
            ...
            <VisualMap state={state} segId={activeSegId} seg={seg}/>
          </>
        ):(
          <>
            <button onClick={()=>onSelectSeg(null)} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(30,52,90,0.6)",borderRadius:6,padding:"5px 11px",cursor:"pointer",fontSize:10,color:"rgba(255,255,255,0.5)",textAlign:"left",width:"fit-content"}}>← All Segments</button>
...
const CAT_TO_NC = Object.fromEntries(Object.entries(NC_TO_CAT).map(([nc,cat])=>[cat,nc]));

/* ─────────────────────────────────────────────────────────
   PARSE IMPORT  — maps external JSON → internal state
───────────────────────────────────────────────────────── */
function parseImport(raw){
  ...
  if(!Array.isArray(notes)||notes.length===0)
    throw new Error("No 'notes' array found in JSON");

  const validSids = new Set(Object.keys(SECTIONS));
  const out = Object.fromEntries(Object.keys(SECTIONS).map(k=>[k,[]]));
  ...
  [...notes]
    ...
    .forEach(n=>{
      ...
      const text = (n.content ?? n.text ?? "").replace(/\n{2,}/g,"\n").trim();
      if(!text) return;
      out[sid].push({
        id:     mkId(),
        text,
        cat:    NC_TO_CAT[n.color] ?? "info",
        segs:   [],
  ...
  return {
    state: out,
    title: parsed.canvas?.title ?? parsed.title ?? null,
    canvasId: parsed.canvas?.id ?? null,
    count: notes.length,
    createdBy: parsed.canvas?.created_by ?? null,
  };
}
...
function ImportModal({open,onClose,onConfirm}){
  ...
  const tryParse = text => {
    try { setPv(parseImport(text)); setErr(""); }
    catch(e){ setErr(e.message); setPv(null); }
  };
  const onFile = e => {
    const f = e.target.files?.[0];
    if(!f) return;
    const r = new FileReader();
    r.onload = ev => { setRaw(ev.target.result); tryParse(ev.target.result); };
    r.readAsText(f);
  };
  const doConfirm = () => {
    if(!preview) return;
    onConfirm(preview, mode);
    setRaw(""); setPv(null); setErr(""); setMode("merge");
    onClose();
  };
  ...
  return(
    <div
      ...
      style={{position:"fixed",inset:0,background:"rgba(1,3,10,0.88)",
        backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",
        zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",
        padding:20,animation:"fadeIn 0.18s ease"}}>
      <div style={{background:"rgba(8,12,24,0.99)",border:"1px solid rgba(60,100,170,0.85)",
        borderRadius:14,width:"100%",maxWidth:520,
        boxShadow:"0 40px 100px rgba(0,0,0,.8)",overflow:"hidden",animation:"fadeUp 0.22s ease"}}>

        {/* Header */}
        <div style={{background:"rgba(255,255,255,0.03)",borderBottom:"1px solid rgba(30,52,90,0.6)",
          ...
          <button onClick={onClose} style={{position:"absolute",top:16,right:18,
            background:"rgba(255,255,255,.04)",border:"1px solid rgba(30,52,90,0.6)",
            color:"rgba(255,255,255,0.5)",width:26,height:26,borderRadius:5,
        ...
        <div style={{padding:"16px 22px 20px",display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div
            ...
            onDrop={e=>{e.preventDefault();const f=e.dataTransfer.files[0];if(f){const r=new FileReader();r.onload=ev=>{setRaw(ev.target.result);tryParse(ev.target.result);};r.readAsText(f);}}}
            style={{border:"1px dashed rgba(255,209,0,0.35)",borderRadius:8,padding:"14px",
              textAlign:"center",cursor:"pointer",background:"rgba(255,209,0,0.04)",
              transition:"background .15s"}}
            onMouseEnter={e=>e.currentTarget.style.background="rgba(255,209,0,0.09)"}
            onMouseLeave={e=>e.currentTarget.style.background="rgba(255,209,0,0.04)"}>
            <div style={{fontSize:22,marginBottom:4}}>📂</div>
            <div style={{fontSize:11,color:"rgba(255,255,255,0.55)"}}>
              Click to browse or drag &amp; drop a <strong>.json</strong> file
            </div>
          ...
          <div>
            <label style={{display:"block",fontSize:9,fontWeight:700,
              ...
              letterSpacing:.8,marginBottom:7,fontFamily:"'JetBrains Mono',monospace"}}>
              Or paste JSON directly
            </label>
            <textarea
              value={raw}
              onChange={e=>{setRaw(e.target.value);tryParse(e.target.value);}}
              placeholder={'{"canvas":{...},"notes":[...]}'}
              style={{width:"100%",background:"rgba(255,255,255,0.04)",
                border:`1px solid ${err?"#ef4444":raw?"rgba(255,209,0,0.45)":"rgba(30,52,90,0.7)"}`,
                color:"#dde6f8",padding:"9px 11px",borderRadius:8,fontSize:11,
                fontFamily:"'JetBrains Mono',monospace",outline:"none",
                resize:"vertical",minHeight:90,lineHeight:1.5,transition:"border-color .15s"}}/>
          </div>
          ...
          {preview&&(
            <div style={{background:"rgba(74,222,128,0.08)",border:"1px solid rgba(74,222,128,0.3)",
              borderRadius:8,padding:"10px 14px",animation:"fadeUp 0.2s ease"}}>
              <div style={{fontSize:10,fontWeight:700,color:"#4ade80",marginBottom:6}}>
                ✓ Ready to import
              </div>
              {preview.title&&(
                <div style={{fontSize:12,color:"#dde6f8",fontWeight:600,marginBottom:4}}>
                  "{preview.title}"
                  {preview.createdBy&&<span style={{fontSize:10,color:"rgba(255,255,255,0.4)",marginLeft:8}}>by {preview.createdBy}</span>}
                </div>
              )}
              <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:8}}>
                {Object.entries(preview.state)
                  .filter(([,v])=>v.length>0)
                  .map(([k,v])=>(
                    <span key={k} style={{display:"inline-flex",alignItems:"center",gap:3}}>
                      <span style={{fontSize:8,fontFamily:"'JetBrains Mono',monospace",
                        background:SECTIONS[k]?.accent,color:"#fff",
                        padding:"1px 5px",borderRadius:2,fontWeight:700}}>
                        {k.toUpperCase()}
                      </span>
                      <span style={{fontSize:9,color:"#dde6f8",fontWeight:600}}>{v.length}</span>
                    </span>
                ))}
              </div>
              {/* Authors found */}
              {(()=>{
                const authors=[...new Set(Object.values(preview.state).flat().map(e=>e.author).filter(Boolean))];
          ...
          {preview&&(
            <div>
              <label style={{display:"block",fontSize:9,fontWeight:700,
                color:"rgba(255,255,255,0.45)",textTransform:"uppercase",
                letterSpacing:.8,marginBottom:8,fontFamily:"'JetBrains Mono',monospace"}}>
                Import Mode
              </label>
              <div style={{display:"flex",gap:6}}>
                {[
                  {id:"merge",   icon:"➕", title:"Add to existing canvas",
                   desc:"New entries are appended to your current cards. Nothing is overwritten."},
                  {id:"replace", icon:"🔄", title:"Replace canvas",
                   desc:"All current entries are cleared and replaced with the imported data."},
                ].map(opt=>(
                  <div
                    key={opt.id}
                    onClick={()=>setMode(opt.id)}
                    style={{flex:1,background:mode===opt.id?"rgba(255,209,0,0.08)":"rgba(255,255,255,0.03)",
                      border:`1px solid ${mode===opt.id?"rgba(255,209,0,0.5)":"rgba(30,52,90,0.6)"}`,
                      borderRadius:8,padding:"10px 12px",cursor:"pointer",transition:"all .15s"}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                      <span style={{fontSize:14}}>{opt.icon}</span>
                      <span style={{fontSize:11,fontWeight:700,
                        color:mode===opt.id?"#ffd100":"#dde6f8"}}>{opt.title}</span>
                      {mode===opt.id&&<span style={{marginLeft:"auto",fontSize:12,color:"#ffd100"}}>✓</span>}
                    </div>
        ...
        {/* Footer */}
        <div style={{display:"flex",gap:7,borderTop:"1px solid rgba(30,52,90,0.6)",
          padding:"12px 22px",background:"rgba(255,255,255,0.02)",flexWrap:"wrap"}}>
          <button onClick={onClose}
            style={{flex:1,background:"transparent",border:"1px solid rgba(30,52,90,0.6)",
              color:"rgba(255,255,255,0.5)",padding:9,borderRadius:8,fontSize:12,
              fontWeight:500,cursor:"pointer",fontFamily:"'Inter',sans-serif",minWidth:80}}>
            Cancel
          </button>
          <button onClick={doConfirm} disabled={!preview}
            style={{flex:2,
              background:preview
                ? mode==="replace"
                  ? "linear-gradient(135deg,#ef4444,#ff7070)"
                  : "linear-gradient(135deg,#ffd100,#ffe566)"
                : "rgba(255,255,255,0.08)",
              border:"none",
              color:preview?(mode==="replace"?"#fff":"#060606"):"rgba(255,255,255,0.3)",
              padding:9,borderRadius:8,fontSize:12.5,fontWeight:800,
              cursor:preview?"pointer":"not-allowed",
              fontFamily:"'Inter',sans-serif",minWidth:140,
              boxShadow:preview?"0 0 18px rgba(255,209,0,0.25)":"none"}}>
            {preview
              ? mode==="replace"
                ? `🔄 Replace Canvas (${Object.values(preview.state).flat().length} entries)`
                : `➕ Add ${Object.values(preview.state).flat().length} entries`
              : "Import"}
          </button>
        </div>
...
export default function App(){
  ...
  const handleImportConfirm=(preview, mode)=>{
    const incomingCount=Object.values(preview.state).flat().length;
    if(mode==="replace"){
      ...
      setState(prev=>{
        ...
        Object.keys(SECTIONS).forEach(k=>{
          merged[k]=[...(prev[k]??[]),...(preview.state[k]??[])];
        });
        return merged;
      });
    }
    if(preview.title) setCanvasTitle(preview.title);
    showToast(
      mode==="replace"
        ? `Canvas replaced — ${incomingCount} entries imported`
        : `${incomingCount} entries added to canvas`
    );
  };
  ...
  const handleExport=useCallback(()=>{
    ...
    const slug=(canvasTitle||"bmc-export").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"");
